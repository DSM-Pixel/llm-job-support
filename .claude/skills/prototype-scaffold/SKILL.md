...
description: Gradio/FastAPI/Streamlit 단일 파일 프로토타입 스캐폴드를 `prototypes/<feature>/` 아래 생성. 시연 시나리오 한 줄과 MOCK 처리 함수로 시작해 점진적으로 실 모델을 끼우게 한다.
allowed-tools:
  - Bash
  - Read
  - Write
...
def run(image, query: str):
    ...
    # TODO(<feature>): 실제 VLM/SAM/RAG 호출
